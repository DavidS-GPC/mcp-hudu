...
export function registerMagicDashTools(server) {
    server.tool('list_magic_dash', 'List Magic Dash widgets in Hudu. Filter by company to see all status indicators for a specific client.', {
        page: z.number().int().min(1).default(1),
        company_id: z.number().int().optional().describe('Filter to a specific company'),
        search: z.string().optional().describe('Filter by widget title'),
    }, async ({ page, company_id, search }) => {
        try {
            const params = { page };
            if (company_id) params.company_id = company_id;
            if (search) params.search = search;
            const { data } = await huduClient.get('/magic_dash', { params });
    ...
    server.tool('update_magic_dash', 'Push a status update to a Hudu Magic Dash widget. Hudu upserts by company + title — if a widget with this title already exists for the company, it will be updated; otherwise a new one is created.', {
        ...
        title: z.string().describe('Widget title/identifier — must match the existing Magic Dash item name exactly, or a new widget will be created'),
        message: z.string().describe('Status message to display on the widget'),
        status: z.enum(['success', 'warning', 'danger', 'info']).describe('Widget colour/state indicator'),
        icon: z.string().optional().describe('FontAwesome icon class, e.g. "fas fa-check"'),
